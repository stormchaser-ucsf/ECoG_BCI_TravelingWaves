...
pvals_move = nan(length(subj_ids),1);
stats_out_move = cell(length(subj_ids),1);

pvals_hold = nan(length(subj_ids),1);
stats_out_hold = cell(length(subj_ids),1);
...
for s = 1:length(subj_ids)
    ...
    [p, h, stats] = signrank(hold_vals);
    pvals_hold(s) = p;
    stats_out_hold{s} = stats;
    fprintf('Subject %d for hold: p = %.4g, zval = %.3f, n = %d\n', ...
        subj_ids(s), p, stats.zval, length(move_vals));
    
    [p, h, stats] = signrank(move_vals);
    pvals_move(s) = p;
    stats_out_move{s} = stats;
    fprintf('Subject %d for move: p = %.4g, zval = %.3f, n = %d\n', ...
        subj_ids(s), p, stats.zval, length(move_vals));
end
...
cd('C:\Users\nikic\Documents\GitHub\ECoG_BCI_TravelingWaves')
ec176 = load('EC176_sig_ch_mu_hG_PAC.mat');
ec210 = load('EC210_sig_ch_Mu_hG_PAC.mat');
ec189 = load('EC189_sig_ch_mu_hG_PAC.mat');
...
d = [hold_per_sig' move_per_sig']*100;
figure;
bar([1 2],mean(d), 'BarWidth', 0.5,'FaceColor',[.75 .75 .75 ])
hold on
idx = ones(size(d,1),1) + 0.1*randn(size(d,1),1);
plot(idx,d(:,1),'.b','MarkerSize',40)
idx2 = 2*ones(size(d,1),1) + 0.1*randn(size(d,1),1);
plot(idx2,d(:,2),'.b','MarkerSize',40)
xticks(1:2)
xticklabels({'Hold','Move'})
yticks(100*[0:.05:1])
xlim([.5 2.5])
for i=1:3
    plot([idx(i) idx2(i)],[d(i,:)],'Color',[.25 .25 .85 .5])
end
plot_beautify
