...
%% ANALYSIS 4.55
% 2nd order linear transformation of data to see if decoding acc. can be
%recovered
% B3 -> no smoothing
% B1, B6 -> smoothing  -> this is more probably to get rid of bad channels
...
%% ANALYSIS 4.6 
% COMPARING DECODING ACC AFTER TRANSFORMING WAVE TO NONWAVE

addpath(genpath('/home/user/Documents/Repositories/ECoG_BCI_HighDim/'))
tid = [condn_data.targetID];
% 
% for i=1:7
%     idx = find(tid==i);
%     Xw = [condn_data(idx).wave_neural];
%     Xnw = [condn_data(idx).nonwave_neural];
% 
%     nt_w=[];
%     nt_nw=[];
%     for j=1:length(condn_data)
%         if condn_data(j).targetID==i
%             tmp=condn_data(j).wave_neural;
%             nt_w = [nt_w size(tmp,2)];
% 
%             tmp=condn_data(j).nonwave_neural;
%             nt_nw = [nt_nw size(tmp,2)];
%         end
%     end
% 
%     Xw_aligned = match_1st_2nd_moments(Xw', Xnw', nt_w, nt_nw);
% 
%     start_idx = 1;k=1;
%     for ii = 1:length(condn_data)
% 
%         if  condn_data(ii).targetID==i
% 
%             T = nt_w(k);
%             idx = start_idx:(start_idx+T-1);
% 
%             % Extract this trial
%             trial_data = Xw_aligned(idx,:);    % T × 253
% 
%             % Transpose back to 253 × T
%             condn_data(ii).wave_neural_aligned = trial_data';
% 
...
condn_data_wave={};
condn_data_nonwave={};
condn_data_nonwave_hat={};
k=1;
for i=1:length(condn_data)
    tmp=condn_data(i).wave_neural';
    tmp1=condn_data(i).nonwave_neural';
    %tmp2 = condn_data(i).wave_neural_aligned';
    targetID = condn_data(i).targetID;
    ...
    condn_data_wave(i).neural = tmp';
    condn_data_wave(i).targetID = targetID;
    
    %condn_data_nonwave_hat(i).neural = tmp2';
    %condn_data_nonwave_hat(i).targetID = targetID;

    if size(tmp,1) < size(tmp1,1)
        idx = randperm(size(tmp1,1),size(tmp,1));
        %idx = 1:size(tmp1,1);
        tmp1 = tmp1(idx,:);
    end

    condn_data_nonwave(i).neural = tmp1';
    condn_data_nonwave(i).targetID = targetID;
end


condn_data_wave = remove_bad_trials(condn_data_wave);
condn_data_nonwave = remove_bad_trials(condn_data_nonwave);
...
for i=1:7
    idx = find(tid==i);
    Xw = [condn_data_wave(idx).neural];
    Xnw = [condn_data_nonwave(idx).neural];

    nt_w=[];
    nt_nw=[];
    for j=1:length(condn_data)
        if condn_data(j).targetID==i
            tmp=condn_data_wave(j).neural;
            nt_w = [nt_w size(tmp,2)];

            tmp=condn_data_nonwave(j).neural;
            nt_nw = [nt_nw size(tmp,2)];
        end
    end
    ...
    for ii = 1:length(condn_data)
        ...
        if  condn_data(ii).targetID==i
            ...
            T = nt_w(k);
            idx1 = start_idx:(start_idx+T-1);

            % Extract this trial
            trial_data = Xw_aligned(idx1,:);    % T × 253

            % Transpose back to 253 × T
            condn_data_nonwave_hat(ii).neural = trial_data';
            condn_data_nonwave_hat(ii).targetID = i;
...
condn_data_nonwave_hat = remove_bad_trials(condn_data_nonwave_hat);

iterations=10;
[acc_wave,train_permutations,acc_bin_wave,bino_pdf,bino_pdf_chance] = ...
    accuracy_imagined_data(condn_data_wave, iterations);
[acc_nonwave,train_permutations,acc_bin_nonwave,bino_pdf,bino_pdf_chance] = ...
    accuracy_imagined_data(condn_data_nonwave, iterations);
[acc_nonwave_hat,train_permutations,acc_bin_nonwave_hat,bino_pdf,bino_pdf_chance] = ...
    accuracy_imagined_data(condn_data_nonwave_hat, iterations);
...
for i=1:size(acc_nonwave,1)
    ...
    tmp=squeeze((acc_nonwave(i,:,:)));
    tmp1=squeeze((acc_wave(i,:,:)));
    tmp2=squeeze((acc_nonwave_hat(i,:,:)));
    %res(i,:) = [mean(diag(tmp0)) mean(diag(tmp)) mean(diag(tmp1))];
    res(i,:) = [ mean(diag(tmp)) mean(diag(tmp1)) mean(diag(tmp2))];
end
figure;boxplot(100*res)
xticks(1:3)
xticklabels({'Non wave epochs', 'Wave epochs', 'Trf. non wave'})
%xticklabels({'Most unstable nowave','Non wave epochs', 'Wave epochs'})
title('Trial Level Acc.')
%signrank(res(:,1),res(:,2))
ylim([40 100])
...
% GETTING DATA FOR TRAINING CNN IN PYTORCH
%(MAIN)
...
%% ANALYSIS 7 (MAIN) 
% getting data in terms of single sample mu complex data over grid (xtrain)
% and the computed 2d phase gradient (ytrain)

% continuation from analysis 6
...
for i=1:length(idx)
    ...
    tmp=all_data_hand(idx(i)).mu_nonwave;
    tmp = cell2mat(tmp');    
    ytrain_tmp=[];
    xtrain_tmp=[];
    parfor j=1:size(tmp,1)
