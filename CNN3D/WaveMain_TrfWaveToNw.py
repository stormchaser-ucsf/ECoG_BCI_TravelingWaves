...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 24 07:33:28 2026
...
waves_recon=[] # bin level
nonwaves_recon=[] # bin level
...
for iterr in np.arange(iterations):    
    ...
    Xtrain,Xval = training_split_wavesAE_recon(condn_data,0.8)
    ...
    if 'model' in locals():
        del model    
    model = iAutoencoder_recon(input_size,hidden_size,latent_dims,num_classes).to(device)        
    ...
    nn_filename = 'iAE_B1_Waves_Recon.pth' 
    ...
    model = training_loop_iAE_waves_recon(model,num_epochs,learning_rate,batch_val,
                          patience,gradient_clipping,nn_filename,
                          Xtrain,Xval,
                          input_size,hidden_size,latent_dims,num_classes)
    
    # transform wave to estimated nonwave data
    nonwave_neural_hat = []    
    model.eval()
    with torch.no_grad():
        for x in condn_data["wave_neural"]:
            input_data = torch.from_numpy(x).to(device).float()
            input_data = input_data.t()
            
            out = model(input_data).t()
            out = out.to('cpu').detach().numpy()
            nonwave_neural_hat.append(out)
    
    # store
    condn_data["nonwave_neural_hat"] = nonwave_neural_hat
