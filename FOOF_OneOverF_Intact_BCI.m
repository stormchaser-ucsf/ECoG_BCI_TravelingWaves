...
% null hypothesis testing for each channel to see if phases are distributed
% uniformly around zero (means no PAC)
pval_hold=[];pval_move=[];
...
% plotting hG and LFOs for a few channel ERPs
figure;
ch=92;
% plot hG
subplot(2,1,1)
hg_ep1 = squeeze((hg_ep(:,ch,:)));
m = mean(hg_ep1,2);
mb = sort(bootstrp(1000,@mean,hg_ep1'));
...
alp_ep1 = squeeze((alp_ep(:,ch,:))); 
m = mean(alp_ep1,2);
mb = sort(bootstrp(1000,@mean,alp_ep1'));
[fillhandle,msg]=jbfill(tt,(mb(25,:)),(mb(975,:))...
        ,[0.2 0.2 0.8],[0.2 0.2 0.8],1,.25);
hold on
plot(tt,m,'Color','b','LineWidth',1)
