function [xdata,ydata,idx,trial_idx] = ...
    get_spatiotemp_windows_VectorField(files,d2,ecog_grid,xdata,ydata,hilbert_flag)
%function [xdata,ydata,idx,trial_idx] = ...
...
for ii=1:length(files)
    ...
    if loaded==1
        ...
        % filter at 1khz
        %df = filtfilt(d1,data);
        for xx=1:19:length(df) %39 is the original step forward
            if size(df,1)-xx > 20
                st = xx;
                stp = xx+19;
                yst=xx+1;
                ...
                parfor j=1:size(tmpx,1)
                    ...
                    % get vector field 
                    [pm,pd,dx,dy] = phase_gradient_complex_multiplication_NN(aa, ...
                        1,-1);
                    ...
                    % get vector field 
                    [pm,pd,dx,dy] = phase_gradient_complex_multiplication_NN(bb, ...
                        1,-1);
                    %[XX,YY] = meshgrid( 1:size(xph,2), 1:size(xph,1) );
                    ph=pd;
                    M =  1.*cos(ph);
