...
if isempty(R) % can only happen if p=0 as we know n>p>=0
    ...
    tol = abs(R(1)) * max(n,p) * eps(underlyingType(R));
end
xrank = nnz(abs(diag(R)) > tol);
if xrank==p
    b(perm,:) = R \ (Q'*y);
else
...
if (nargout>1)
   ...
   tempC = (RI * RI') * sigma^2;
   tempse = sqrt(max(eps(underlyingType(tempC)),diag(tempC)));
   C = NaN(p,p,"like",tempC);
   covb = zeros(p,p,"like",tempC);
   se = zeros(p,1,"like",tempse);
...
[b,r] = internal.stats.linsolve(xw,yw,struct('RECT',true));
