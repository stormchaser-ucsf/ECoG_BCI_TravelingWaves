...
%subj_dir = sprintf('%s/%s/', subjects_dir, subj); NN
subj_dir = sprintf('%s/', subjects_dir);

%load(sprintf('%s/Meshes/%s_%s_pial.mat', subj_dir, subj, hem)); NN
load(sprintf('%s/%s_%s_pial.mat', subj_dir, subj, hem));
%load(sprintf('%s/elecs/%s.mat', subj_dir,elecfile_prefix)); NN
load(sprintf('%s/%s.mat', subj_dir,elecfile_prefix));
...
for i=1:length(brain_areas)
    ...
    el_add(elecmatrix(inds,:), 'color', elec_color, 'numbers', inds_labels, 'msize', 8);
