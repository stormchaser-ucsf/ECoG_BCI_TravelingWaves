function [W, min_cost, Ahat, Shat] = CEBM( X )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% entropy bound minimization," IEEE Trans. Circuits and Systems I, vol. 57, 
% no. 7, pp. 1417-1430, July 2010. 
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Program by Xi-Lin Li, MLSP Lab
%
...
for iter = 1 : maxiter_sea
    ...
    for n = 1 : N
        ...
        z_real = real(z);
        z_imag = imag(z);
        sigma_R2 = sum( z_real.^2 )/T;
        sigma_I2 = sum( z_imag.^2 )/T;
        sigma_R = sqrt(sigma_R2);
        sigma_I = sqrt(sigma_I2);
        rho = sum( z_real.*z_imag )/T;
        ...
        Cost(iter) = Cost(iter) + 0.5*log(Delta1) + log(2*pi) + 1;
...
if show_cost
    ...
    title('Initialization','FontSize',MyFontSize,'FontName','Arial')
...
for iter = 1 : max_iter_north
    ...
    for n = 1 : N
        ...
        negentropy_array(n) = -0.5*log(Delta1);         % we consider the negentropy
        ...
        if type2 == 0 || bound_real > max_NEr
            ...
            switch p0
                ...
                case 1
                    ...
                    grad = grad - vEGu*X*( 4*weight.*uu.*u )'/sum(weight)/2/sigma_R;
                    grad = grad - vEGu*sum( -4*weight.*uu.*u.*z_real )/sum(weight)/4/sigma_R^3*R_xxt*conj(w);
                ...
                case 3
                    ...
                    grad = 0.5*grad_delta1/Delta1;
                    grad = grad - vEGu*X*( weight.*sign_u./(1+abs_u).^2 )'/sum(weight)/2/sigma_R;
                    grad = grad - vEGu*sum( -weight.*sign_u./(1+abs_u).^2.*z_real )/sum(weight)/4/sigma_R^3*R_xxt*conj(w);
                ...
                case 5
                    ...
                    grad = 0.5*grad_delta1/Delta1;
                    grad = grad - vEGu*X*( weight.*abs_u.*(20+abs_u)./(10+abs_u).^2 )'/sum(weight)/2/sigma_R;
                    grad = grad - vEGu*sum( -weight.*abs_u.*(20+abs_u)./(10+abs_u).^2.*z_real )/sum(weight)/4/sigma_R^3*R_xxt*conj(w);
                ...
                case 7
                    ...
                    grad = 0.5*grad_delta1/Delta1;
                    grad = grad - vEGu*X*( weight.*(1-u.*u)./(1+u.*u).^2 )'/sum(weight)/2/sigma_R;
                    grad = grad - vEGu*sum( -weight.*(1-u.*u)./(1+u.*u).^2.*z_real )/sum(weight)/4/sigma_R^3*R_xxt*conj(w);
            ...
            switch q0
                ...
                case 1
                    ...
                    grad = grad + vEGv*X*( 4*weight.*vv.*v )'/sum(weight)/2/sigma_R/sqrt(Delta1)*(rho+sqrt(-1)*sigma_R2);
                    grad = grad + vEGv*sum( 4*weight.*vv.*vv )/sum(weight)/2/Delta1*grad_delta1;
                    grad = grad - vEGv*sum( 4*weight.*vv.*v.*( z_imag + (rho/sigma_R2+2*sqrt(-1))*z_real ) )/sum(weight)/4/sigma_R/sqrt(Delta1)*R_xxt*conj(w);

                case 3
                    % G = |x|/(1+|x|)
                    % g = sign(x)/(1+|x|)^2
                    vEGv = simplified_ppval( real_nf3.pp_slope, min(max(EGv(3), real_nf3.min_EGx), real_nf3.max_EGx) );
                    grad = grad + vEGv*X*( weight.*sign_v./(1+abs_v).^2 )'/sum(weight)/2/sigma_R/sqrt(Delta1)*(rho+sqrt(-1)*sigma_R2);
                    grad = grad + vEGv*sum( weight.*abs_v./(1+abs_v).^2 )/sum(weight)/2/Delta1*grad_delta1;
                    grad = grad - vEGv*sum( weight.*sign_v./(1+abs_v).^2.*( z_imag + (rho/sigma_R2+2*sqrt(-1))*z_real ) )/sum(weight)/4/sigma_R/sqrt(Delta1)*R_xxt*conj(w);

                case 5
                    % G = x|x|/(10+|x|)
                    % g = |x|(20+|x|)/(10+|x|)^2
                    vEGv = simplified_ppval( real_nf5.pp_slope, min(max(EGv(5), real_nf5.min_EGx), real_nf5.max_EGx) );
                    grad = grad + vEGv*X*( weight.*abs_v.*(20+abs_v)./(10+abs_v).^2 )'/sum(weight)/2/sigma_R/sqrt(Delta1)*(rho+sqrt(-1)*sigma_R2);
                    grad = grad + vEGv*sum( weight.*abs_v.*(20+abs_v)./(10+abs_v).^2.*v )/sum(weight)/2/Delta1*grad_delta1;
                    grad = grad - vEGv*sum( weight.*abs_v.*(20+abs_v)./(10+abs_v).^2.*( z_imag + (rho/sigma_R2+2*sqrt(-1))*z_real ) )/sum(weight)/4/sigma_R/sqrt(Delta1)*R_xxt*conj(w);
                    
                case 7
                    % G = x/(1+x^2)
                    % g = (1-x^2)/(1+x^2)^2
                    vEGv = simplified_ppval( real_nf7.pp_slope, min(max(EGv(7), real_nf7.min_EGx), real_nf7.max_EGx) );
                    grad = grad + vEGv*X*( weight.*(1-v.*v)./(1+v.*v).^2 )'/sum(weight)/2/sigma_R/sqrt(Delta1)*(rho+sqrt(-1)*sigma_R2);
                    grad = grad + vEGv*sum( weight.*(1-v.*v)./(1+v.*v).^2.*v )/sum(weight)/2/Delta1*grad_delta1;
                    grad = grad - vEGv*sum( weight.*(1-v.*v)./(1+v.*v).^2.*( z_imag + (rho/sigma_R2+2*sqrt(-1))*z_real ) )/sum(weight)/4/sigma_R/sqrt(Delta1)*R_xxt*conj(w);
            ...
            switch max_i
                ...
                case 1
                    ...
                    grad = 0.5*(  1/Delta1 + 1/Delta1^2*vEGr*sum(4*weight.*rr.*Delta2)/sum(weight)  )*grad_delta1;
                    grad = grad - 0.5*1/Delta1*vEGr* X*( 4*weight.*rr.*grad_delta2_part1 ).'/sum(weight);
                    grad = grad  - 0.5*1/Delta1*vEGr* sum( 4*weight.*rr.*grad_delta2_part2 )/sum(weight)*R_xxt*conj(w);
                
                case 3
                    % G = r/(1+r)
                    vEGr = simplified_ppval( nf3.pp_slope, min(max(EGr(3), nf3.min_EGr), nf3.max_EGr) );
                    grad = 0.5*(  1/Delta1 + 1/Delta1^2*vEGr*sum(weight.*inv_r./(1+r).^2.*Delta2)/sum(weight)  )*grad_delta1;
                    grad = grad - 0.5*1/Delta1*vEGr* X*( weight.*inv_r./(1+r).^2.*grad_delta2_part1 ).'/sum(weight);
                    grad = grad  - 0.5*1/Delta1*vEGr* sum( weight.*inv_r./(1+r).^2.*grad_delta2_part2 )/sum(weight)*R_xxt*conj(w);
...
if show_cost
    ...
    hold on; plot(Cost([1:iter]), '-b', 'MarkerSize', MyMarkerSize, 'linewidth', MyLineWidth);
    box on;
    %hold on; plot([1:iter], min_cost_queue([1:iter]),'r-');
    xlabel('Number of iterations','FontSize',MyFontSize,'FontName','Arial')
    ylabel('Cost','FontSize',MyFontSize,'FontName','Arial')
    title('Nonorthogonal ICA','FontSize',MyFontSize,'FontName','Arial')
end
%[EOF]
