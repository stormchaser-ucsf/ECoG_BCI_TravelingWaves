...
Dmean_CL = median(D_CL,3);

[Y_CL,eig_CL] = cmdscale(Dmean_CL,3);
...
Yd = Y_CL(:,1:3);
K = 5;
idx = kmeans(Yd, K, 'Replicates', 100);
...
ImaginedMvmt = {'Thumb','Index','Middle','Ring','Pinky','Power',...
    'Pinch','Tripod','Wrist In','Wrist Out','Wrist Flex','Wrist Extend'};
figure;
[Y,~,disparities] = mdscale(Donline,2);
Ymds=Y;
hold on
...
% plottig dendrogram
Z=linkage(squareform(Donline),'complete');
figure;
dendrogram(Z,0)
...
for i=1:length(session_data) % 20230518 has the best hand data performance
    ...
    for ii=1:length(folders)
        ...
        %cd(folderpath)
        files = [files;findfiles('',folderpath)'];
        [acc_batch,acc_batch_bin,trial_len,bino_pdf,decodes_trial,trial_acc] = ...
            accuracy_online_data_Hand(files,12);
        ...
        acc_CL_bins(kk).foldername = folders{ii};
        kk=kk+1;
        num_trials = [num_trials ;[length(files) i]];
        trial_level_acc(jj).day = i;
        trial_level_acc(jj).num_trials = length(files);
        trial_level_acc(jj).decodes = trial_acc;
        jj=jj+1;
    end
...
% compare accuracy across trials for early vs. late days
early_days = [];
late_days = [];
for i=1:length(trial_level_acc)
    if trial_level_acc(i).day <=3
        early_days = [early_days trial_level_acc(i).decodes];
    end

    if trial_level_acc(i).day >=9
        late_days = [late_days trial_level_acc(i).decodes];
    end
end

% bootstrap test of difference in means
stat = mean(late_days) - mean(early_days);
boot=[];
tmp = [early_days late_days];
for i=1:1000
    tmp1 = tmp(randperm(numel(tmp)));
    a = tmp1(1:length(early_days));
    b = tmp1(length(a)+1:end);
    boot(i) = mean(b) - mean(a);
end
figure;hist(boot)
vline(stat)
pval = max(1/length(boot),sum(boot>stat)/length(boot));
...
cilow = [aa(25) bb(25)];
cihigh = [aa(975) bb(975)];

lowerErr = means - cilow;
upperErr = cihigh - means;

x = 1:2;
figure;
errorbar(x, means*100, lowerErr*100, upperErr*100, ...
    'o', ...
    'LineStyle','none', ...
    'MarkerSize',10, ...
