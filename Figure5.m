...
clf
v = VideoWriter('wave_example2_tmp.avi','Motion JPEG AVI');
v.FrameRate = 20;
v.Quality=100;
open(v);
data_wav=[];
...
%% GET MU SIGNAL
% load a file, filter etc. and then visualize

tmp = real(df(:,ecog_grid(:)));
tmp = zscore(tmp);
kindata = tmp(30:100,1:20);
kindata1=kindata;
% visualization
offset = 5 * (1:size(kindata,2));
offset=repmat(offset,size(kindata,1),1);
temp=kindata+offset;
figure;plot(temp,'Color','k','LineWidth',1)
axis tight
plot_beautify
axis off

%% GETTING EXAMPLE OF A ROTATION TRAVELING WAVE FROM A SMALL PORTION OF THE GRID

cd('C:\Users\nikic\Documents\GitHub\ECoG_BCI_TravelingWaves\mat_plots\WaveExamples')
files = findfiles('.fig',pwd)';
tt = 100:10:230;
filepath = 'C:\Users\nikic\Documents\GitHub\ECoG_BCI_TravelingWaves\mat_plots\WaveExamples\svg\';
for i=1:length(tt)
    for j=1:length(files)
        if contains(files{j},num2str(tt(i)))
            break
        end
    end
    uiopen(files{j},1)
    filename = [num2str(tt(i)) 'ms.svg'];
    filename = fullfile(filepath,filename);
    set(gcf,'PaperPositionMode','auto');
    print(gcf,filename,'-dsvg','-painters','-r300');
end
...
res_B1 = get_wave_summary_stats(stats_cl_days);
...
res_B3 = get_wave_summary_stats(stats_cl_days);
...
res_B6 = get_wave_summary_stats(stats_cl_days);
...
xlim([0 5])
xlabel('Number of traveling waves/sec (Freq.)')
...
xticks ''
plot_beautify
ylabel('Traveling wave duration (ms)')


%%%% better scatter plot of duty cycle
b1_acc = res_B1.duty_cycle_days;
...
% svg
set(gcf,'PaperPositionMode','auto');
print(gcf,'B1_WaveFreqHist.svg','-dsvg','-painters','-r300');
