...
for i=1:5%6:length(session_data)
    folders_imag =  strcmp(session_data(i).folder_type,'I');
    folders_online = strcmp(session_data(i).folder_type,'O');
    folders_batch = strcmp(session_data(i).folder_type,'B');
    folders_batch1 = strcmp(session_data(i).folder_type,'B1');
    ...
    for ii=1:length(files)
        ...
        if loaded==1
            ...
            % filter at 1khz
            %df = filtfilt(d1,data);
            for xx=1:39:length(df) %50ms step forward
                if size(df,1)-xx > 42
                    st = xx;
                    stp = xx+39;
                    yst=xx+1;
...
save alpha_dynamics_200Hz_1st_5Days_Rawzscore xdata ydata labels labels_batch days -v7.3
