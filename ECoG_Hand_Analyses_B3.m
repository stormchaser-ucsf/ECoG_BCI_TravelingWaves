...
%% FINDING BEST PERFORMANCE BLOCK IN ONLINE CONTROL 
...
for i=1:length(session_data)
    ...
    if hg_alpha_switch
        ...
        [xdata,ydata,idx,trial_idx] = ...
            get_spatiotemp_windows_VectorField(files,d2,ecog_grid,xdata,ydata,1);
    end
...
%% loading data, filtering and extracting epochs for use in a CNN AE, ROI BY ROI  
% MAIN
...
for i=1:length(session_data)
    ...
    if hg_alpha_switch
        ...
        [xdata,ydata,idx,trial_idx] = get_spatiotemp_windows_roi(files,d2,ecog_grid,xdata,ydata,rows,cols);
...
save alpha_dynamics_200Hz_AllDays_STG_ArtifactCorr xdata ydata labels labels_batch days -v7.3
%save hg_LFO_dynamics_hG_200Hz_AllDays_DaysLabeled_ArtifactCorr xdata ydata labels labels_batch days -v7.3
